...
# my packages
from recommender.recommender.framework.tf2.utils.model_helpers import past_stop_threshold
from recommender.recommender.framework.tf2.utils.hooks_helper import get_logging_tensor_hook
from recommender.recommender.framework.tf2.utils.parsers import BaseParser
...
class WideDeepArgParser(argparse.ArgumentParser):  # 定义WideDeepArgParser类，用于解析参数
    ...
    def __init__(self):  # 初始化函数
        super(WideDeepArgParser, self).__init__(parents=[BaseParser()])  # 调用父类的初始化函数
        self.add_argument(
        ...
        self.set_defaults(  # 为其他参数设置默认值
            data_dir='/Users/ericdoug/Documents/datas/adult',  # 设置数据样本路径
...
def trainmain(argv):
    ...
    for n in range(flags.train_epochs):  # 将总迭代数，按照epochs_between_evals分段。并循环对每段进行训练#调用估算器的train方法进行训练
        ...
        if past_stop_threshold(  # 根据accuracy的阈值，来判断是否需要结束训练。
